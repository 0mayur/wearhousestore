...
const CartScreen = ({ match, location, history }) => {
  ...
  const dispatch = useDispatch(); //dispatch method with use of our usedisptach

  const cart = useSelector((state) => state.cart); // all the item from the cart how we read it from the state in use selector we will get our entire state and from that state.cart
  ...
  return (
    <Grid gridTemplateColumns="3">
      <Box>
        ...
        <Flex>
          {cartItems.length === 0 ? ( //we want display cart items
            <Message>
              Your cart is empty.{" "}
